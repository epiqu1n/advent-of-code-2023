...
def calculate_button_hold_times(race_time: int, best_distance: int) -> range:
    ...
    base_time = floor(race_time / 2)
    low_bound = base_time
    high_bound = base_time
    new_low = -1
    new_high = -1
    iteration = 1

    def calc_new_time(base_time: int, prev_time: int, iteration: int, direction: 'low' or 'high') -> int:
        adjustment = base_time / (2 ** iteration)
        return floor(prev_time + adjustment if direction == 'high' else prev_time - adjustment)

    while True:
        new_low = calc_new_time(base_time, low_bound, iteration, 'low')
        new_high = calc_new_time(base_time, high_bound, iteration, 'high')
